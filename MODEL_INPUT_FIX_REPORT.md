...
## 20. WITHDRAWN — brand analysis handed to a dedicated agent

**The brand analysis implementation was removed from this branch** (owner scope
change, to avoid two implementations of one thing). Removed in full:
`src/training/brand_analysis.py`, `tests/test_brand_analysis.py`,
`BrandAnalysisSpec` / `BrandPairRow` / `BRAND_PAIR_COLUMNS`,
`evaluation.brand_analysis` in `config/training.yaml`, and the
`brand_analysis_pairs` artifact path + `DataConfig` field. A grep for
`brand_analysis|BrandAnalysisSpec|BrandPairRow|BRAND_PAIR_COLUMNS` over
`src/`, `config/` and `tests/` returns nothing.

**Brand-string normalisation was also reverted** and is NOT applied. The
accent folding was removed from `core.model_input._normalized_tokens`, so the
composition is back to `normalize_text` alone; the decimal percentage marker
returns to `pct5.5` (the dot survives `normalize_text`, which preserves
`[a-z0-9.]`). `legacy` was re-verified after the revert: **0 byte-mismatches
over all 855 fixture rows × 2 sides**.

### 20.1 The measurements are kept as INPUT to the deliberate decision

The evidence below was produced by execution before the withdrawal. It is
recorded here because the fix should be assigned on evidence, not re-derived —
and because one finding contradicts the obvious hypothesis.

**Accent handling is broken, and worse than "unnormalised".** `normalize_text`
deletes every non-ASCII character, so an accent does not merely go unfolded —
it becomes a word break that corrupts the token:
...
`Reál` and `Réal` are the *same brand* and tokenise to *different* tokens, so
two spellings can never match. **47 distinct canonical brands carry
non-ASCII.** The repo already owns the fix:
`core.critical_attributes.normalized_attribute_text` (NFKD + casefold + strip
combining marks) is its one accent-folding normaliser, already used by
`core.hard_negatives.normalized_product_name` — any patch should call it
rather than add a second implementation. Folding it in is a one-line change in
`_normalized_tokens`; it was measured to work and then reverted on instruction.

**But accent folding would change almost nothing today.** Over the catalog,
1,655 distinct brands collapse into exactly **3** variant clusters under
folding + corporate-suffix stripping — `REAL | Reál | Réal` (8 GTINs),
`ECO | Eco+` (14), `Viva | Viva!` (6) — **28 GTINs, 0.21 %**, and **0 labelled
pairs** involve two spellings of one brand. So the accent defect is real but
currently latent.

**The 585-pair review band, where brands DO differ** (388 of 585 mismatch):
197 exact matches, **378 genuinely different brands**, 10 pairs in the
ambiguous 0.60–0.85 ratio band with **0 surface variants, 0 empty brands and
0 title-only brands**. All 10 ambiguous pairs were inspected individually and
every one is a *different* brand sharing a substring (`Radnor`/`Rainbow`
0.615, `LIFEWTR`/`ZenWTR` 0.615, `Cemilefendi`/`Cemil` 0.625, `Peace Tea`/
`Seven Teas` 0.632, `Thick- It`/`Thick & Easy` 0.667, `Réal`/`Realemon` 0.667,
`Albi`/`Marli` 0.667) — none is a normalisation failure.

**Consequence for the analysis agent:** brand mismatch on the review band is a
**retrieval** problem (378 of 388 are outright different brands), not a
string-composition problem. Any classification that reports a large
"surface variant" class on this population should be re-examined against these
counts.

## 21. STATUS — provenance / contract stamping (item 5)

**Two of the four requested fields are present; two are not.** Another agent
working this repository in parallel introduced
`TrainingSpec.ModelInputComposition` (pydantic, `src/core/schemas.py`) carrying
`profile`, `include_evidence` and a `fingerprint` digest, stamped into the run
trace, the checkpoint manifest, the prepared-bundle manifest and the ANN reuse
fingerprint through the existing `core.tracing` / `config/paths.yaml`
machinery.

| requested | status |
|---|---|
| composition profile (`cleaned`/`legacy`) | **present** (concurrent work) |
| config hash / fingerprint | **present** (concurrent work) |
| symmetry mode | **NOT present** |
| code commit (git SHA) | **NOT present** |

I did not add the missing two: the area was under active concurrent edit, and
extending another agent's in-flight schema unverified is how two schemas for
one concept appear. **Open.**
