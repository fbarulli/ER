...
Run:  python src/training/selftest.py
...
def oracle_precision_at_recall() -> None:
    ...
    from training.training import _precision_at_recall
...
def oracle_ranking_at_k() -> None:
    """Top-K metrics are stable, correctly normalized, and hit-aware."""
    from core.ranking_metrics import ranking_at_k

    metrics = ranking_at_k(
        np.array([0, 1, 0, 1]), np.array([0.9, 0.8, 0.7, 0.6]), (1, 5, 10)
    )
    check("Hits@1 is 0 when rank 1 is irrelevant", metrics["hits_at_1"] == 0.0)
    check("Precision@1 is top-rank relevance", metrics["precision_at_1"] == 0.0)
    check("Recall@5 sees both positives", metrics["recall_at_5"] == 1.0)
    check("Precision@5 uses available four candidates", metrics["precision_at_5"] == 0.5)
...
def oracle_phrase_variants() -> None:
    ...
    from pipeline import NgramIDF, extract_discriminative_ngrams
    ...
    def keeps(title: str) -> set[str]:
        from pipeline import PHRASE_VARIANTS
...
def oracle_word_once() -> None:
    ...
    folds = dict(vocabulary()["CONCEPT_FOLDS"])
...
def oracle_config_split() -> None:
    ...
    config/paths.yaml is now TWO files, each validated by its pydantic model
    ...
    EDA keys now live in config/training.yaml)."""
    ...
    check("config/training.yaml exists", TRAINING_CONFIG_PATH.exists())
    check("config/vocabulary.json exists", VOCABULARY_CONFIG_PATH.exists())
    ...
    check("vocabulary has both stopword lists and macros", all(
        key in vocabulary()
...
def oracle_no_fallback_ssot() -> None:
    ...
    # src/training/hpo.py reads the SAME lists (no divergent module-level copy)
    import training.hpo as _hpo_mod

    check(
        "src/training/hpo.GRID == config hpo.grid",
        _hpo_mod.GRID == h["grid"] and _hpo_mod.QUICK == h["quick"],
    )
    # src/training/training.HPO_SPACE == config tpe_space
    import training.training as _tr_mod

    check(
        "src/training/training.HPO_SPACE == config hpo.tpe_space",
    ...
    #    plot dpi now live in config/training.yaml
    t2 = training_cfg()
    ...
    banned = {
        ...
        "src/training/training.py": [r'HPO_SPACE\s*=\s*\{\s*\n\s*"epochs":'],
        "src/training/rerank.py": [r"max_length\s*=\s*512", r"d_pr\s*>\s*0\.005"],
...
def oracle_round3_pins() -> None:
    ...
    from core.common import F, sweep_cfg, training_cfg
    ...
    from core.common import _CFG  # the merged view
    ...
    import training.blocking_audit as _ba

    check(
        "src/training/blocking_audit.BUDGET == audit.blocking_budget",
        _ba.BUDGET == a.blocking_budget,
        f"got {_ba.BUDGET}",
    )
    check(
        "src/training/blocking_audit.MIN_RECALL == audit.blocking_min_recall",
        _ba.MIN_RECALL == a.blocking_min_recall,
        f"got {_ba.MIN_RECALL}",
    )

    # ── strip-audit ladder bands pin (SSOT move, this round): the band
    #    edges moved from an inline list in src/training/strip_audit.py into
    #    audit.strip_ladder_bands — SAME VALUES, new home, so the pin
    #    proves neither the values nor the move drifted
    from core.common import strip_ladder_bands
    ...
    #    taxonomy is DOMAIN DATA moved from src/core/text.py into config/paths.yaml
    #    category_macros: — pin the exact 24-category shape + the module
    #    copy stays deleted
    from core.common import category_macros, load_dataset_deduped
    ...
    import core.common as _lc3
    import core.nlp as _ln
...
def oracle_zero_pack_guard() -> None:
    ...
    for title, want in cases:
        got = pipeline.extract_all(title, "")["pack_qty"]
        check(
    ...
    for title, want in (
        ...
        got = pipeline.extract_pack_from_title(title)[0]
...
def oracle_manifest() -> None:
    ...
    pipeline run):
      1. results/ absent or EMPTY (core.common mkdirs it
    ...
    if not manifest_path.exists():
        check(
            ...
            "PYTHONPATH=src python3 -m training.dedupe",
        )
        return
    try:
