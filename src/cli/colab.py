...
def run_hpo(mode: str | None = None) -> None:
    ...
    print(
        "[run] round-robin HPO -> held-out evaluation -> CrossEncoder rerank "
        f"(mode={mode}, workers={_HPO_WORKERS}) ..."
    )
    script = _BOOTSTRAP + _wandb_env_script() + f"""
...
def prepare_worker(model_key):
    if mode != "parallel_same_vm":
        return root / "results", {{}}
    worker_results = root / "results" / "hpo_workers" / model_key
    worker_results.mkdir(parents=True, exist_ok=True)
    # The frozen CSVs are shared inputs; mutable result trees are isolated.
    for name in (F["canonical_records"], F["gate_results"]):
        source = root / "results" / name
        target = worker_results / name
        if not source.is_file():
            raise FileNotFoundError(f"worker input missing: {{source}}")
        shutil.copy2(source, target)
    env = {{
        "EUROMONITOR_RESULTS_DIR": str(worker_results),
        "EUROMONITOR_MLRUNS_DIR": str(worker_results / "mlruns"),
        "WANDB_RUN_NAME": f"hpo_{{model_key}}",
    }}
    return worker_results, env

def run_model(model_key):
    model = resolve_model(model_key)
    worker_results, worker_env = prepare_worker(model_key)
    model_tag = str(model).rstrip("/").rsplit("/", 1)[-1]
    best_path = worker_results / f"train_{{model_tag}}-dlr_hpo_best.json"
    print(f"== HPO {{model_key}}: {{model}} (dev-selected; test withheld)", flush=True)
    run_logged(base + ["--model", str(model), "--hpo"], f"hpo_{{model_key}}", worker_env)
    ...
    return {{"model_key": model_key, "model": str(model), "best": params,
            "results_dir": str(worker_results.relative_to(root / "results"))
...
if mode == "parallel_same_vm":
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(workers, len(model_keys))) as pool:
        summary = [future.result() for future in
                   [pool.submit(run_model, key) for key in model_keys]]
else:
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=_HPO_MODE,
        help="HPO backbone scheduling mode (default from config/training.yaml)",
    )
    ap.add_argument(
        "--refresh-data",
        action="store_true",
