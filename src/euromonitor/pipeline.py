"""euromonitor.pipeline.py — THE official data pipeline, all transformations in ONE
...
Sections (in dependency order):
  ...
  3. similarity    — jaccard (the old embedding-similarity stub was dead:
                     zero consumers; the zero-shot lane is src/euromonitor/training/zero_shot_sims)
...
from euromonitor.ner.ner_product_attributes import extract_title_attributes
...
def three_way_gate(
    ...
    live in src/euromonitor/training/training.yaml `gate:` and are read through training_cfg()
...
def load_verdicts() -> dict[str, str] | None:
    ...
    CSV is written by src/euromonitor/training/build_reference.py, not mutated mid-run.
    """
...
def run_within_brand_pipeline(
    ...
    # data-prep regeneration (src/euromonitor/training/data_prep.py is the sole caller) and
...
def build_training_data(
    ...
    *,
    ...
    # crosses into src/euromonitor/training/train + src/euromonitor/training/training; a shape break must die
    # HERE with a named field, not as an IndexError in a fold.
    from euromonitor.core.schemas import TrainingData as _TrainingData
