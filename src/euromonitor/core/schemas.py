...
TWO jobs:
  ...
  1. CONFIG CONTRACTS — one pydantic model per split config file, validated
     at load by lib.common (fail-loudly at import, never mid-run):
       ...
       TrainingConfig   src/euromonitor/training/training.yaml — the training lane's knobs
     (EDA/eda.yaml was deleted with the EDA dir 2026-09-10; its five
     TRAIN-consumed keys — plots.dpi, pairs.max_pos_per_group/n_neg/
     neg_oversample, strip_audit_sample — migrated into src/euromonitor/training/training.yaml.)
  ...
  2. BOUNDARY CONTRACTS — validated containers at every transform boundary
     in the pipeline (small objects, never per-row hot loops):
       ExtractedAttributes   euromonitor.pipeline.extract_all
       GateResult            euromonitor.pipeline.three_way_gate
       CanonicalRecord       euromonitor.pipeline.generate_canonical
       PairArrays            index-pair containers (pos/neg/hp)
       TrainingData          euromonitor.pipeline.build_training_data (the bundle)
...
class DataConfig(BaseModel):
    ...
    live in their own dir: src/euromonitor/training/training.yaml."""
...
class SplitSpec(BaseModel):
    """Component-aware split contract (src/euromonitor/training/training.yaml split:).
...
class EvaluationSpec(BaseModel):
    """Zero-shot evaluation protocol (src/euromonitor/training/training.yaml evaluation:) —
...
class MaskingSpec(BaseModel):
    """Masking augmentation contract (src/euromonitor/training/training.yaml masking:)."""
...
class TrainingSpec(BaseModel):
    """Training runtime knobs (src/euromonitor/training/training.yaml training:).
...
class PairsSpec(BaseModel):
    """Pair-label thresholds + eval-pair caps (src/euromonitor/training/training.yaml pairs:) —
...
class TrainingPlotsSpec(BaseModel):
    """Plot rendering (src/euromonitor/training/training.yaml plots:) — the one DPI every
...
class AuditSpec(BaseModel):
    """Audit-lane knobs (src/euromonitor/training/training.yaml audit:) — strip-audit sample
    ...
    src/euromonitor/training/blocking_audit.py; audit round 2 F18, moved round 3) + the
...
class GateSpec(BaseModel):
    """Three-way-gate decision thresholds (src/euromonitor/training/training.yaml gate:) — the
    gate's decision table. Formerly signature defaults in
    euromonitor.pipeline.three_way_gate (audit round 2, F01): vol_tolerance (relative
...
class BandsSpec(BaseModel):
    """Cosine bands (src/euromonitor/training/training.yaml bands:) — eval/rerank.
...
class MiningSpec(BaseModel):
    """Hard-negative mining knobs (src/euromonitor/training/training.yaml mining:) — `band` is
...
class HpoGridRowSpec(BaseModel):
    """One fixed-grid config (src/euromonitor/training/training.yaml hpo.grid[] / hpo.quick[]):
...
class HpoSpaceSpec(BaseModel):
    """TPE search ranges (src/euromonitor/training/training.yaml hpo.tpe_space:) — each value is
...
class HpoSpec(BaseModel):
    """HPO lane knobs (src/euromonitor/training/training.yaml hpo:) — the fixed grid, the
    --quick smoke subset, the TPE space, and trial budgets. Formerly
    module-level literals in src/euromonitor/training/hpo.py / src/euromonitor/training/training.py."""
...
class RerankSpec(BaseModel):
    """07e A/B decision rule (src/euromonitor/training/training.yaml rerank:) — the quantitative
    ...
    src/euromonitor/training/rerank.py."""
...
class SweepSpec(BaseModel):
    """Ablation-sweep axes (src/euromonitor/training/training.yaml sweep:) — run_all step 4's
...
class TrainingConfig(BaseModel):
    """src/euromonitor/training/training.yaml — the training lane's OWN config (in its dir).
...
class EvalSummaryRow(BaseModel):
    ...
    src/euromonitor/training/evaluate_models.py, validated BEFORE the CSV write.
