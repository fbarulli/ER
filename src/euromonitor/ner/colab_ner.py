...
def load_settings() -> dict:
    ...
    if not base_dir.is_absolute():
        base_dir = (HERE / base_dir).resolve()
    ...
    remote_project_dir = f"{remote_root}/{HERE.name}"
...
def upload_inputs(settings: dict) -> None:
    ...
    colab_upload(settings, CONFIG_PATH, f"{remote}/config.yaml")
    colab_upload(settings, HERE / "ner.py", f"{remote}/ner.py")

    config_loader = HERE / "config_loader.py"
    if config_loader.is_file():
        colab_upload(settings, config_loader, f"{remote}/config_loader.py")
