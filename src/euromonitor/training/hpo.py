"""src/euromonitor/training/hpo.py — hyperparameter sweeps (owner's second07 semantics + masking).
...
Two lanes, both on the OFFICIAL clean-sku pair scheme:
  ...
  tpe   (second08 lane): optuna TPE over HPO_SPACE, resume-safe sqlite
        study, per-fold mean AUC objective — src/euromonitor/training/training.run_hpo.
...
Writes (results/):
  ...
  train_<model><era>_hpo_trials.csv    run-tagged names built inline in
                                       src/euromonitor/training/training.run_hpo, which prints
                                       the actual out_path)
"""
...
from euromonitor.core.common import RESULTS, SEED, F, set_determinism
from euromonitor.core.common import SSOT_LOSS as _SSOT_LOSS  # no-fallback SSOT

# second07's grid semantics (epochs x lr x warmup%), SSOT: src/euromonitor/training/training.yaml
...
from euromonitor.core.common import hpo_cfg as _hpo_cfg
from euromonitor.core.common import runtime as _runtime  # SSOT optimizer knobs (F06)
from euromonitor.training.training import ES_PATIENCE, ES_THRESHOLD, _band_tuple, train_one_config
...
def run_grid(
    ...
    Masking: applied ONCE by the entry lane (src/euromonitor/training/train.py resolves
    ...
    # (same pad src/euromonitor/training/train.main does before building the data tuple)
    import numpy as _np
    ...
    for cfg in grid:
        ...
        tcfg = {
            ...
            # src/euromonitor/training/train.py's cfg build. The old inline 0.01/"linear"/1.0
...
def run_tpe(
    ...
    """
    from euromonitor.training.training import run_hpo

    # determinism at LANE entry (2026-10-06): both sweep lanes get the
    # same pin even when invoked directly — the entry lane (src/euromonitor/training/train
...
def _mlf_null():
    ...
    from euromonitor.training.training import MlflowCtx
