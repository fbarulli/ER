...
TRAIN_GPU root; the training internals live in src/euromonitor/training/).
...
from euromonitor.core.common import SSOT_LOSS as _SSOT_LOSS
from euromonitor.training.folds import component_folds
from euromonitor.training.training import ES_PATIENCE, ES_THRESHOLD, train_one_config

# thresholds live in 00_config.yaml (pairs.proceed_sim_threshold /
# pairs.hardneg_sim_threshold) and are read by euromonitor.pipeline.build_training_data
...
def load_training_data(df: pd.DataFrame, payload_variant: str = "full") -> dict:
    ...
    from euromonitor.pipeline import build_training_data
...
def _emit_07_series(ok_rows: list[dict], args) -> None:
    ...
    src/euromonitor/training/train invocation adds its variant's row, so run_all's sweep
    ...
    from euromonitor.core.common import RESULTS
...
def main() -> None:
    ...
    from euromonitor.core.mlflow_ctx import MlflowCtx
    from euromonitor.core.wandb_ctx import WandbCtx
...
def _main_inner(_mlf, _wandb) -> None:
    ...
    from euromonitor.core.common import resolve_model
    ...
    # ── masking augmentation (src/euromonitor/training/masking.py, on/off via 00_config) ──
    ...
    # (U(0.20, 0.35) per masked copy — src/euromonitor/training/training.yaml masking band;
    ...
    if args.mask_frac > 0:
        from euromonitor.training.masking import augment_positives
        ...
        from euromonitor.core.common import write_visibility_log as _wvl
    ...
    if args.split == "holdout":
        ...
        dev_override = dev_bc
        from euromonitor.core.hard_negatives import pairs_in_set
    ...
    #       present — loud notice when absent, see lib/volume_verified.py);
    #   (b) the pipeline's proceed-pairs (gate-confirmed same volume+pack) —
    #       these ARE train_all's positives by construction.
    from euromonitor.core.volume_verified import volume_verified_cross_country
    ...
    from euromonitor.core.common import DATA_DIR
    ...
    if args.grid or args.hpo:
        from euromonitor.training.hpo import run_grid, run_tpe
    ...
    # src/euromonitor/training/training's DEFAULT_CFG key set (train_one_config reads these)

    # NO FALLBACK (owner Q27): every optimizer knob comes from the SSOT
    # training: block via runtime() — the inline 0.05/0.01/linear/1.0
    # literals duplicated src/euromonitor/training/training.yaml and could silently diverge.
    cfg = {
        "epochs": args.epochs,
        "lr": args.lr,
    ...
    # ── 07-series CSV emission (owner ruling: emit from src/euromonitor/training/train) ────────
            ...
            _wandb.log_metrics(
                {
                    ...
                    for k in (
                        "auc", "auc_cross", "acc_at_thr", "pr_auc",
    ...
    if args.plot and aucs:
        from euromonitor.training.plots import report_plots, training_loss_plot
    ...
    if args.rerank:
        from euromonitor.training.rerank import rerank_stage
