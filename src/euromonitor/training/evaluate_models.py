...
components of the positive-pair barcode graph (src/euromonitor/training/folds.
component_folds; k / dev_fold / test_fold from src/euromonitor/training/training.yaml
...
TEST as the leak diagnostic only (same convention as src/euromonitor/training/training.py
...
if not EMBED_SIM_CSV.exists():
    raise SystemExit(
        f"{EMBED_SIM_CSV.name} missing — run src/euromonitor/training/zero_shot_sims.py first"
    )
...
if _n_after == 0:
    raise SystemExit(
        ...
        "does not cover labeled_pairs (rerun src/euromonitor/training/zero_shot_sims.py)"
    )
...
# connected components of that graph (src/euromonitor/training/folds.component_folds): every
...
if _pos_straddle:
    raise AssertionError(
        ...
        f"component guarantee is broken (src/euromonitor/training/folds.component_folds)"
    )
...
if _pos_dev == 0 or _neg_dev == 0:
    raise ValueError(
        ...
        f"dev_fold or component_split_k in src/euromonitor/training/training.yaml)"
    )
if _pos_test == 0 or _neg_test == 0:
    raise ValueError(
        ...
        f"test_fold or component_split_k in src/euromonitor/training/training.yaml)"
    )
