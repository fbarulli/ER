...
Stages (identical order + code paths as euromonitor.pipeline.clean_sku_text — this
...
Usage:
  python src/euromonitor/training/strip_audit.py --gtin 747519482230        one SKU, full ladder
  python src/euromonitor/training/strip_audit.py --sample 200              random SKUs, summary
  python src/euromonitor/training/strip_audit.py --sample 200 --levels 8   + similarity ladder
...
def ladder(title: str, attribute: str = "", brand: str = "") -> dict:
    ...
    from euromonitor.pipeline import spell_numeric_brand, strip_number_tokens
...
def main() -> None:
    ...
    # default sample SSOT: src/euromonitor/training/training.yaml audit.strip_audit_sample
    # (was inline 200, then EDA/eda.yaml strip_audit_sample)
    from euromonitor.core.common import training_cfg as _training_cfg
    ...
    if args.levels:
        ...
        # src/euromonitor/training/training.yaml (validated by AuditSpec: contiguous ascending
