...
class CrossBrandMiningSpec(BaseModel):
    """Cross-brand hard-negative miner knobs (mining.cross_brand:).

    The lane mines pairs whose brands DIFFER while every other critical
    attribute agrees, so the encoder has to use brand as the discriminator.
    ``require_agreement`` is the BLOCKING key: a canonical carrying no explicit
    evidence for one of those dimensions generates no candidate, and the two
    sides must agree on the value. Every other critical dimension is still
    required to be conflict-free by the shared evaluator — the list is the
    candidate-GENERATION rule, not the whole agreement rule.
    """
    ...
    enabled: bool
    target: int = Field(ge=0)
    min_similarity: float = Field(ge=0.0, lt=1.0)
    require_agreement: list[str] = Field(min_length=1)
    max_per_canonical: int = Field(ge=0)
    max_per_brand: int = Field(ge=0)

    @field_validator("require_agreement")
    @classmethod
    def _critical_unique_dimensions(cls, values: list[str]) -> list[str]:
        ...
        if unknown:
            raise ValueError(
                "mining.cross_brand.require_agreement names non-critical "
                f"dimensions {unknown}; allowed: {sorted(allowed)}"
            )
        if len(set(values)) != len(values):
            raise ValueError(
                f"mining.cross_brand.require_agreement repeats a dimension: {values}"
            )
        return values
...
class MiningSpec(BaseModel):
    ...
    cross_brand: CrossBrandMiningSpec
...
class TrainingStats(BaseModel):
    ...
    # Cross-brand lane: candidates entering the miner's funnel (candidate pairs
    # generated by the require_agreement blocking) and the pair rows it
...
class TrainingData(BaseModel):
    ...
    cross_brand_neg: np.ndarray = Field(
        default_factory=lambda: np.empty((0, 2), dtype=int)
    )
