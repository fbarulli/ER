...
# pairs.hardneg_sim_threshold) and are read by data_pipe.build_training_data
...
def _emit_07_series(ok_rows: list[dict], args) -> None:
    ...
    TRAIN/train invocation adds its variant's row, so run_all's sweep
...
def _main_inner(_mlf) -> None:
    ...
    from lib.common import resolve_model
    ...
    if args.mask_frac > 0:
        ...
        from lib.common import write_visibility_log as _wvl
    ...
    # literals duplicated TRAIN/training.yaml and could silently diverge.
    cfg = {
        "epochs": args.epochs,
        "lr": args.lr,
    ...
    # ── 07-series CSV emission (owner ruling: emit from TRAIN/train) ────────
