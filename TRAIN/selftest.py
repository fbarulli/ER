...
def oracle_gtin() -> None:
    from lib.gtin import barcode_validity, is_valid_gtin_checksum
...
def oracle_cleaning() -> None:
    from data_pipe import MODEL_PAYLOAD_SOFT_STOP, normalize_text, strip_schema_words
...
def oracle_number_reference() -> None:
    from lib.common import DATA_DIR, F
...
def oracle_mining() -> None:
    from lib.hard_negatives import mine_hard_negatives
...
def oracle_holdout_integrity() -> None:
    ...
    import data_pipe
    from lib.common import SEED, load_dataset_deduped
    from TRAIN.folds import component_folds

    df = load_dataset_deduped()
    d = data_pipe.build_training_data(df)
...
def oracle_precision_at_recall() -> None:
    ...
    from TRAIN.training import _precision_at_recall
...
def oracle_youden_discipline() -> None:
    ...
    the accuracy)."""
    from TRAIN.training import _youden_thr
...
def oracle_append_csv() -> None:
    from TRAIN.train import _append_csv
...
def oracle_phrase_variants() -> None:
    ...
    from data_pipe import NgramIDF, extract_discriminative_ngrams
    ...
    def keeps(title: str) -> set[str]:
        from data_pipe import PHRASE_VARIANTS
...
def oracle_word_once() -> None:
    ...
    folds = dict(
        _json.loads(
            (RESULTS.parent.parent / "lib" / F["stopwords"]).read_text()
        )["CONCEPT_FOLDS"]
    )
...
def oracle_pinned_counts() -> None:
    ...
    try:
        ...
        check(
            "gate decisions hard_no=92,650 proceed=29,351 fallback=13,768",
            ...
            # reproducibly yields.
            dec == {"hard_no": 92650, "proceed": 29351, "fallback": 13768},
            f"got {dec}",
        )
        check(
            "labeled pairs == 19,916 (7,409 pos / 12,507 hard-neg)",
            # same audit as above: +10 hard-negs vs the 2026-09-07 census
            # (pack_set [0] -> [1] lets those pairs compare packs honestly)
            len(lp) == 19916 and (lp.true_label == 1).sum() == 7409,
...
def oracle_config_split() -> None:
    ...
    EDA keys now live in TRAIN/training.yaml)."""
    ...
    check("TRAIN/training.yaml exists", TRAINING_CONFIG_PATH.exists())
    ...
    check(
        "files.stopwords points at lib/pipe_stopwords.json",
        data_cfg().files.stopwords == "pipe_stopwords.json",
    )
    from pathlib import Path as _P

    _lib = _P(data_cfg().files.stopwords).parent
    check(
        "lib/pipe_stopwords.json exists",
        (CONFIG_PATH.parent / "lib" / "pipe_stopwords.json").exists(),
    )
    check(
        "lib/sklearn_stopwords.json exists",
        (CONFIG_PATH.parent / "lib" / "sklearn_stopwords.json").exists(),
    )
...
def oracle_no_fallback_ssot() -> None:
    ...
    # TRAIN/hpo.py reads the SAME lists (no divergent module-level copy)
    import TRAIN.hpo as _hpo_mod

    check(
        "TRAIN/hpo.GRID == config hpo.grid",
        _hpo_mod.GRID == h["grid"] and _hpo_mod.QUICK == h["quick"],
    )
    # TRAIN/training.HPO_SPACE == config tpe_space
    import TRAIN.training as _tr_mod

    check(
        "TRAIN/training.HPO_SPACE == config hpo.tpe_space",
    ...
    #    plot dpi now live in TRAIN/training.yaml
    t2 = training_cfg()
    ...
    banned = {
        ...
        "TRAIN/training.py": [r'HPO_SPACE\s*=\s*\{\s*\n\s*"epochs":'],
        "TRAIN/rerank.py": [r"max_length\s*=\s*512", r"d_pr\s*>\s*0\.005"],
        "TRAIN/train.py": [
        ...
        "TRAIN/evaluate_models.py": [r"dpi\s*=\s*150"],
    }
...
def oracle_round3_pins() -> None:
    ...
    from data_pipe import three_way_gate
    from lib.common import F, sweep_cfg, training_cfg
    ...
    from lib.common import _CFG  # the merged view
    ...
    import TRAIN.blocking_audit as _ba

    check(
        "TRAIN/blocking_audit.BUDGET == audit.blocking_budget",
    ...
    import lib.common as _lc3
    import lib.nlp as _ln
...
def oracle_zero_pack_guard() -> None:
    ...
    for title, want in cases:
        got = data_pipe.extract_all(title, "")["pack_qty"]
        check(
    ...
    for title, want in (
        ...
        got = data_pipe.extract_pack_from_title(title)[0]
